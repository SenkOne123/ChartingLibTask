...
export const getExchangeServerTime = () => request('/time').then(res => {
	console.log(`Server time is ${new Date(res.serverTime)}`)
	return new Date()
...
export const getKlines = ({ symbol, interval, from, to, limit }) => {
	...
	return request('/klines', { symbol: symbol.toUpperCase(), interval, startTime: from, endTime: to, limit: limit })
		.then(res => {
			return res.map(i => ({
				time: i.t*1000,
				open: i.o,
...
function request(url, params = {}) {
	return axios({
		baseURL: 'https://charting-lib-server.herokuapp.com/',
		method: 'get',
		url,
		params
	})
